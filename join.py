...
wiki_path = "file:///app/output/wiki_kdrama_final_parquet"
...
wiki_df = (
    ...
    # .filter(length(col("wiki_title_clean")) > 4)  
    # .withColumn("korean_count", size(split(lower(col("text")), r"korean|k-drama")) - 1)
    # .filter(col("korean_count") >= 10)
...
# crawler_df = crawler_df.withColumn("title_words", split(col("title_clean"), " "))
# wiki_df = wiki_df.withColumn("wiki_words", split(col("wiki_title_clean"), " "))
...
# join_title = crawler_df.crossJoin(wiki_df).filter(
#      expr("title_clean RLIKE CONCAT('(^|[^a-z0-9])', wiki_title_clean, '([^a-z0-9]|$)')")
#     #   | array_contains(col("tags"), col("wiki_title_clean"))
# ).withColumn("match_type", lit("title"))

# join_title_raw = crawler_df.crossJoin(wiki_df).filter(
#     expr("lower(title_clean) LIKE concat('%', lower(wiki_title_clean), '%')")
# )
...
output_path = "file:///app/output/joined1_parquet"
