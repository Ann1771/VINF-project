...
def clean_wikitext(val):
    ...
    val = re.sub(r"\[\[(?![Ff]ile:|[Ii]mage:)(?:[^\|\]]*\|)?([^\]]+)\]\]", r"\1", val, flags=re.I) 
    val = re.sub(r"\{\{[Ii]nterlanguage link\|([^|}]+)[^{}]*?\}\}", r"\1", val, flags=re.I)  
    val = re.sub(r"\{\{\s*ko-translit\|[^{}]*?\}\}", "", val, flags=re.I) 
    val = re.sub(r"\{\{\s*[Bb]ased on\|([^|{}]+)\|([^{}]+)\}\}", r"\1 by \2", val, flags=re.I)  
    ...
    val = re.sub(r"\[\[(?:[Ff]ile|[Ii]mage):[^\]]*\]\]", "", val, flags=re.I | re.S) 
...
def extract_kdrama_data(partition):
    ...
    for page in re.findall(r"<page>(.*?)</page>", xml, re.S):
        ...
        synopsis_match = re.search(r"=+\s*Synopsis\s*=+\s*(.*?)(?===|$)", text, re.S | re.I)
        if not synopsis_match:
            synopsis_match = re.search(r"=+\s*Plot\s*=+\s*(.*?)(?===|$)", text, re.S | re.I)
...
output_dir = "/app/output/wiki_kdrama_final1_parquet"
